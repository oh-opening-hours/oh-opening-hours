...
namespace OH_Opening_Hours\Fields;

/**
 * Abstraction for a FieldRenderer
 *
 * @author      Jannik Portz
 * @package     OH_Opening_Hours\Fields
 */
class FieldRenderer {
  /**
  ...
  protected function renderField(array $field, $value = null) {
    ...
    /** Field Label */
    if (!empty($caption) && !in_array($type, array(FieldTypes::CHECKBOX, FieldTypes::HEADING))) {
    ...
    switch ($type) {
      ...
      case FieldTypes::URL:
        ...
        printf('<input type="%s" id="%s" name="%s" value="%s" %s />', esc_attr($type), esc_attr($id), esc_attr($name), esc_attr($value), esc_attr($attrString));
      ...
      case FieldTypes::TEXTAREA:
        $attrString = $this->generateAttributesString($attributes);
        printf('<textarea id="%s" name="%s" %s>%s</textarea>', esc_attr($id), esc_attr($name), esc_attr($attrString), esc_textarea($value));
        break;
    ...
    if (array_key_exists('description', $field)) {
      printf('<span class="op-field-description">%s</span>', esc_html($field['description']));
    }

    if (isset($description) && is_string($description)) {
      echo '<span class="op-widget-description">' . esc_html($description) . '</span>';
    }
