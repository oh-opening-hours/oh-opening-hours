...
namespace OH_Opening_Hours\Entity;
 ...
 *
 * @author      Jannik Portz
 * @package     OH_Opening_Hours\Entity
 */
class Holiday implements TimeContextEntity, DateTimeRange {
  ...
  private function setDateUniversal($date, $property, $end_of_day = false) {
    if (
      is_string($date) &&
      (preg_match(Dates::STD_DATE_FORMAT_REGEX, $date) ||
    ...
    if (!$date instanceof DateTime) {
      function ohopoh_datetime_notice() {
        $class = 'notice notice-warning';
        $message = __( sprintf( 'Argument one for %s has to be of type string or DateTime, %s given', __CLASS__ . '::' . __METHOD__, gettype($date)), 'oh-opening-hours' );
        printf( '<div class="%1$s"><p>%2$s</p></div>', esc_attr( $class ), esc_html( $message ) );
      }
      add_action( 'admin_notices', 'ohopoh_datetime_notice' );
    }
