...
abstract class AbstractShortcode extends AbstractModule {
  const FILTER_ATTRIBUTES = 'opoh_shortcode_attributes';

  const FILTER_TEMPLATE = 'opoh_shortcode_template';
  const FILTER_TEMPLATE_PATH = 'opoh_shortcode_template_path';

  const FILTER_SHORTCODE_MARKUP = 'opoh_shortcode_markup';
  ...
  public function registerShortcode() {
    ...
    try {
      ...
      function opoh_shortcode_notice_error() {
        $class = 'notice notice-error';
        $message = $e->getMessage();
        printf( '<div class="%1$s"><p>%2$s</p></div>', esc_attr( $class ), esc_html( $message ) );
      }
      add_action('admin_notices', 'opoh_shortcode_notice_error');
    }
  }
  ...
  public function renderShortcodeTemplate(array $attributes, $templatePathRel) {
     ...
     * @deprecated                                Use the absolute `opoh_shortcode_template_path` instead
    ...
    $templatePathAbs = sprintf('%s/views/%s', opoh_plugin_dir_path(), $templatePathRel);
  ...
  public function setShortcodeTag($shortcodeTag) {
    $this->shortcodeTag = apply_filters('opoh_shortcode_tag', $shortcodeTag);
  }
